<%@ page import="java.util.*" %>
<html>
    <head>
        <title>Sistema de Averias</title>
        <style type="text/css">
            <%@ include file="css/reset.css" %>
            <%@ include file="css/all.css" %>
        </style>
    </head>
    <body>
        <div id="wrapper">
            <div  class="header">
                <div class="logo"><a href="<%=request.getContextPath()%>/index.html"><img src="<%=request.getContextPath()%>/images/logo.png" /></a></div>
                <div class="nav">
                    <ul>
                        <li><a href="<%=request.getContextPath()%>/index.html">Inicio</a></li>
                        <li><a href="/averias/listaSuscriptores">Suscriptores</a></li>
                        <li><a href="/averias/listaSucursales">Sucursales</a></li>
                        <li class="active"><a href="/averias/listaServicios">Servicios</a></li>
                        <li><a href="/averias/listaAverias">Averias</a></li>
                    </ul>
                </div>
            </div>
            <div class="content">
                <h2>Listado de servicios</h2>
                <% List servicios = (List) request.getAttribute("servicios");%>
                <table class="list">
                    <tr><th>Nombre</th><th>Codigo Region</th><th>Codigo Suscriptor</th><th>Acciones</th></tr>
                    <% for (int i = 0, n = servicios.size(); i < n; i++) {
                                                Map servicio = (Map) servicios.get(i);%>
                    <tr><td><%= servicio.get("nombre")%></td>
                        <td><%= servicio.get("region_codRegion")%></td>
                        <td><%= servicio.get("suscriptor_idSuscriptor")%></td>						
                        <td><a href='/averias/detalleServicio?codServicio=<%= servicio.get("codServicio")%>'>
                                <input type="submit" value="Detalle"/></a>
                            <a href='/averias/eliminarServicio?codServicio=<%= servicio.get("codServicio")%>'>
                                <input type="submit" value="Eliminar"/></a></td></tr>
                                <% }%>
                </table>
                <table>
                    <tr><td><a href='/averias/nuevaServicio'>
                                <input type="submit" name="action" value="Agregar"/></a>
                        </td></tr>
                </table>
            </div>
        </div>
    </body>
</html>